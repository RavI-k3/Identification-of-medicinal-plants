...
# pip install flask flask-cors tensorflow==2.12.0 numpy pymongo google-generativeai
...
def get_gemini_response(predicted_label):
    ...
    prompt = f"""You are a plant expert. Provide a concise and informative summary of the plant {predicted_label} including:
    - Common name and scientific name (if available).
    - Key medical uses and benefits.
    - Any precautions or warnings.
    - A fun fact (optional).
    - List 2-3 related plants (similar in appearance, uses, or habitat).

    - dont start anything like Okay, here's a concise overview of Aloe vera: just provide only neccessary
    Keep the response under 150 words. Use bullet points for clarity and avoid lengthy paragraphs.
    """
